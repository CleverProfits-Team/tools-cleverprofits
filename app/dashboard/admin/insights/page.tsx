...
async function getInsightsData() {
  ...
  const overlapClusters = toolsWithOverlap
    .filter(
      (t) => Array.isArray(t.aiOverlapWarnings) && (t.aiOverlapWarnings as string[]).length > 0,
    )
    .map((t) => ({
      id: t.id,
      name: t.name,
      slug: t.slug,
      overlaps: t.aiOverlapWarnings as string[],
    }))
  ...
  return {
    ...
    frameworkRows: frameworkRows.map((r) => ({
      name: r.aiFrameworkGuess!,
      count: r._count.aiFrameworkGuess,
    })),
  }
}
...
function SummaryCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-5">
      ...
      <p className="text-xs text-[rgba(4,11,77,0.40)] mt-1">{description}</p>
    </div>
  )
}
...
function DistributionBar({
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-[rgba(4,11,77,0.40)] py-4 text-center">No data yet</p>
  }
  return (
    <div className="space-y-2.5">
      {rows.map(({ name, count }) => (
        <div key={name} className="flex items-center gap-3">
          <span className="text-sm text-[rgba(4,11,77,0.55)] capitalize w-36 flex-shrink-0 truncate">
            {name}
          </span>
          <div className="flex-1 bg-[#FAFAFA] rounded-full h-2">
            <div
              className={`h-2 rounded-full ${colorClass}`}
              style={{ width: `${Math.round((count / maxCount) * 100)}%` }}
            />
          </div>
          <span className="text-xs text-[rgba(4,11,77,0.40)] w-6 text-right tabular-nums">
            {count}
          </span>
        </div>
      ))}
...
export default async function InsightsPage() {
  const { abandonedTools, ownerlessTools, overlapClusters, categoryRows, frameworkRows } =
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[rgba(4,11,77,0.55)] mt-1">
          Organizational intelligence — tool health, usage patterns, and gaps.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-6">
          <div className="flex items-center gap-2 mb-5">
            <GitBranch className="h-4 w-4 text-[rgba(4,11,77,0.40)]" aria-hidden />
            <h2 className="text-sm font-semibold font-display text-[#040B4D]">Tool categories</h2>
          </div>
          <DistributionBar
            rows={categoryRows}
            maxCount={maxCategoryCount}
            colorClass="bg-[#2605EF]"
          />
        </div>
        <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-6">
          <div className="flex items-center gap-2 mb-5">
            <Cpu className="h-4 w-4 text-[rgba(4,11,77,0.40)]" aria-hidden />
            <h2 className="text-sm font-semibold font-display text-[#040B4D]">
              Frameworks &amp; tech
            </h2>
          </div>
          <DistributionBar
            rows={frameworkRows}
            maxCount={maxFrameworkCount}
      ...
      <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-6">
        ...
        <p className="text-xs text-[rgba(4,11,77,0.40)] mb-4">
          Active tools with no proxy hits in the last 30 days
        </p>

        {abandonedTools.length === 0 ? (
          <p className="text-sm text-[rgba(4,11,77,0.40)] py-4 text-center">
            All active tools have recent activity.
          </p>
        ) : (
              ...
              <div key={tool.id} className="py-3 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[rgba(4,11,77,0.40)] mt-0.5">
                    {tool.team ? `Team: ${tool.team}` : 'No team'} · {tool.createdByName} · since{' '}
                    {formatDate(tool.createdAt)}
                  </p>
                </div>
                <Link
                  href={`/dashboard/admin/tools/${tool.id}`}
                  className="text-xs font-medium text-[rgba(4,11,77,0.40)] hover:text-[#2605EF] transition-colors flex-shrink-0 focus-visible:ring-2 focus-visible:ring-[#2605EF]/30 focus-visible:ring-offset-2 rounded"
                >
                  Review →
                </Link>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-6">
        ...
        <p className="text-xs text-[rgba(4,11,77,0.40)] mb-4">
          Active or pending tools with no team assigned
        </p>

        {ownerlessTools.length === 0 ? (
          <p className="text-sm text-[rgba(4,11,77,0.40)] py-4 text-center">
            All tools have a team assigned.
          </p>
        ) : (
          <div className="divide-y divide-[#E7E7E7]/60">
      ...
      {overlapClusters.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#E7E7E7] shadow-card p-6">
          ...
          <p className="text-xs text-[rgba(4,11,77,0.40)] mb-4">
            Tools that AI flagged as potentially duplicating existing tools
          </p>
          <div className="divide-y divide-[#E7E7E7]/60">
            {overlapClusters.map((tool) => (
              <div key={tool.id} className="py-3">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="text-xs text-[rgba(4,11,77,0.40)]">may overlap with:</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {tool.overlaps.map((name, i) => (
                    <span
                      key={i}
                      className="text-xs bg-violet-50 text-violet-700 rounded-full px-2 py-0.5 font-medium"
                    >
                      {name}
                    </span>
                  ))}
