...
export default function IdentifyPage() {
  ...
  const checkSlug = useCallback((value: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/tools/check-slug?slug=${encodeURIComponent(value)}`, {
          signal: ctrl.signal,
        })
  ...
  function validate() {
    ...
    if (slugState === 'taken' || slugState === 'invalid')
      errs.slug = slugReason ?? 'Slug is unavailable'
    if (!url.trim()) {
    ...
    try {
      ...
      if (!res.ok) {
        if (data?.issues?.slug?.[0] || res.status === 409) {
          setErrors((p) => ({
            ...p,
            slug: data?.issues?.slug?.[0] ?? 'This slug is already taken',
          }))
        } else {
          setServerErr(
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center">
            <span className="flex-shrink-0 h-9 px-3 flex items-center border border-r-0 border-[#E7E7E7] rounded-l-md bg-[#FAFAFA] text-[rgba(4,11,77,0.40)] text-sm font-mono select-none">
        ...
        <div>
          ...
          {errors.url ? (
            ...
            <p className="text-xs text-[rgba(4,11,77,0.40)] mt-1.5">
              The Railway (or any HTTPS) URL. Never exposed to end users.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.github ? (
            ...
            <p className="text-xs text-[rgba(4,11,77,0.40)] mt-1.5">
              Used to read the README and tech stack for AI analysis.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.description ? (
            ...
            <p className="text-xs text-[rgba(4,11,77,0.40)] mt-1.5">
              2–3 sentences — the most important input for AI analysis.
            </p>
          )}
        </div>
