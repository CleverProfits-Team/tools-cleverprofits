...
function RoleBadge({ role }: { role: Role }) {
  ...
  return (
    <span
      className={cn('inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold', styles[role])}
    >
      {role.replace('_', ' ')}
    </span>
  )
      ...
      <div className="w-full max-w-md rounded-xl border border-[#E7E7E7] bg-white shadow-card p-8">
        {isInvalid ? (
          <>
            ...
            <Link href="/dashboard" className="text-sm text-[#2605EF] hover:underline">
              Go to dashboard →
            </Link>
          </>
        ) : (
          <>
            ...
            <h1 className="text-lg font-bold font-display text-[#040B4D] mb-1">
              You&apos;ve been invited
            </h1>
            ...
            {session ? (
              ...
              <>
                <p className="text-xs text-[rgba(4,11,77,0.40)] mb-5">
