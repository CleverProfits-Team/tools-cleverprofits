...
export function AdminUsersPanel({ initialUsers, currentUserId, currentUserRole }: Props) {
  ...
  return (
    <div className="space-y-8">
      {/* ── Users table ──────────────────────────────────────────────────── */}
      {users.length === 0 ? (
        <p className="text-sm text-[#64748b] py-8 text-center">No users</p>
      ) : (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-[#e2e8f0]/60">
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id} className="hover:bg-[#f4f3f3]/50 transition-colors duration-150">
                    <td className={tdCls}>
                    ...
                    <td className={tdCls}>
                      {editable ? (
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <ShieldAlert className="h-3.5 w-3.5 text-[#94a3b8]" aria-hidden />
                        </div>
                      )}
                    </td>
                    <td className={tdCls}>
                        ...
                        <div>
                          <button
                            ...
                            className={cn(
                              ...
                              user.status === 'ACTIVE'
                                ? 'border border-[#e2e8f0] text-[#64748b] hover:bg-[#f4f3f3]'
      ...
      <div className="rounded-xl border border-[#e2e8f0] bg-white p-6 shadow-card">
        <h3 className="text-sm font-semibold font-display text-[#040B4D] mb-4">Create invitation</h3>
        <form onSubmit={handleInviteSubmit} className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-xs font-medium font-display text-[#94a3b8] uppercase tracking-wide mb-1">Email</label>
            <input
              type="email"
              required
              value={inviteEmail}
              ...
              className="w-full rounded-lg border border-[#e2e8f0] px-3 py-1.5 text-sm text-[#040B4D] placeholder:text-[#94a3b8] focus:outline-none focus:ring-2 focus:ring-[#2605EF]/25 focus:border-[#2605EF]/60 transition-colors"
            />
          </div>
          <div>
            <label className="block text-xs font-medium font-display text-[#94a3b8] uppercase tracking-wide mb-1">Role</label>
            <Select
              value={inviteRole}
              options={getRoleOptions(currentUserRole)}
